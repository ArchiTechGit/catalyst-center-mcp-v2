...
from src.core.mcp_server import CatalystCenterMCP
...
_mcp_instance: Optional[CatalystCenterMCP] = None
...
async def get_mcp_instance() -> CatalystCenterMCP:
    """Get or create the MCP server instance."""
    global _mcp_instance, _mcp_initialized

    if _mcp_instance is None:
        _mcp_instance = CatalystCenterMCP()  # No cluster binding; resolved per-request
...
async def mcp_sse_post(
    ...
    try:
        ...
        if mcp_request.method == "initialize":
            ...
            # Add catalyst_list_clusters utility tool if user has multiple clusters
            if auth_result.user and auth_result.user.clusters and len(auth_result.user.clusters) > 1:
                ...
                filtered_tools.append({
                    "name": "catalyst_list_clusters",
                    "description": f"List your assigned Catalyst Center clusters. You have access to: {', '.join(cluster_names)}",
            ...
            elif tool_name == "catalyst_list_clusters":
