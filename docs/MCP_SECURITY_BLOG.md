...
```
+------------------+     +------------------+     +-------------------+
|   Claude/GPT     |---->|   MCP Server     |---->|  Catalyst Center  |
...
+-------------------+    |  - MCP SSE Endpoint      |
    ...
    | - User/Roles    |    | - 1296 Operations|    | - SSL Optional  |
     +-----------------+    +-----------------+    +-----------------+
```
...
**The Problem:** Catalyst Center credentials stored in plaintext can be extracted from database dumps.
...
**Result:** Database breach does not expose Catalyst Center credentials. Key rotation possible without data loss.
...
SESSION_CONFIG = {
    ...
    "cookie_name": "catalyst_center_mcp_session",
    "max_age": 3600,  # 1 hour
...
- [Catalyst Center MCP Server](https://github.com/ArchiTechGit/catalyst-center-mcp-v2) - Full implementation
...
*This document summarizes the security implementation of the Catalyst Center MCP Server. For deployment instructions, see [DEPLOYMENT.md](DEPLOYMENT.md). For quick start, see [QUICKSTART.md](../QUICKSTART.md).*
