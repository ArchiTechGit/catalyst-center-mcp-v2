...
This document describes the REST API endpoints for LDAP configuration and user cluster assignment added to the Catalyst Center MCP Server.
...
response = session.put(
    ...
    cookies={"catalyst_session": token}
)
print(response.json())
```
...
curl -X GET http://localhost:8000/api/users/1/clusters \
  -H "Cookie: catalyst_session=YOUR_TOKEN"
